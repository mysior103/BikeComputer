...
void OLED_SetCursor(POINT Xpoint, POINT Ypoint) {
	if ((Xpoint > sOLED_DIS.OLED_Dis_Column)
			|| (Ypoint > sOLED_DIS.OLED_Dis_Page))
		return;
...
void OLED_SetWindow(POINT Xstart, POINT Ystart, POINT Xend, POINT Yend) {
	if ((Xstart > sOLED_DIS.OLED_Dis_Column)
			|| (Ystart > sOLED_DIS.OLED_Dis_Page)
			|| (Xend > sOLED_DIS.OLED_Dis_Column)
			|| (Yend > sOLED_DIS.OLED_Dis_Page))
		return;
...
void OLED_SetColor(POINT Xpoint, POINT Ypoint, COLOR Color) {
	...
	if (Xpoint % 2 == 0) {
		Buffer[Xpoint / 2 + Ypoint * 64] = (Color << 4)
				| Buffer[Xpoint / 2 + Ypoint * 64];
	} else {
		Buffer[Xpoint / 2 + Ypoint * 64] = (Color & 0x0f)
				| Buffer[Xpoint / 2 + Ypoint * 64];
	}
}
...
void OLED_Clear(COLOR Color) {
	...
	for (i = 0; i < sOLED_DIS.OLED_Dis_Page; i++) {
		for (m = 0; m < (sOLED_DIS.OLED_Dis_Column / 2); m++) {
			Buffer[i * (sOLED_DIS.OLED_Dis_Column / 2) + m] = Color
					| (Color << 4);
		}
	}
}
...
/********************************************************************************
 ...
 Clear Window
 ********************************************************************************/
void OLED_ClearWindow(POINT Xstart, POINT Ystart, POINT Xend, POINT Yend,
COLOR Color) {
...
void OLED_DisWindow(POINT Xstart, POINT Ystart, POINT Xend, POINT Yend) {
	...
	//write data
	COLOR *pBuf = (COLOR *) Buffer + Xstart
			+ Ystart * (sOLED_DIS.OLED_Dis_Column / 2);
	for (page = 0; page < Ypoint; page++) {
		for (Column = 0; Column < Xpoint; Column++) {
			OLED_WriteData(*pBuf);
			pBuf++;
		}
		pBuf = (COLOR *) Buffer + Xstart
				+ (Ystart + page + 1) * (sOLED_DIS.OLED_Dis_Column / 2);
	}
}
